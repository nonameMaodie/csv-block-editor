...
- **Undo/Redo**（撤销/重做）
  （新增 `Ctrl+Z` 撤销和 `Ctrl+Y` / `Ctrl+Shift+Z` 重做功能，支持单元格编辑、替换单个和全部替换操作的撤销/重做，全部替换作为一次批量操作统一撤销/重做。最多保留 100 条历史记录。）

- **Enhanced cell navigation**（增强单元格导航）
  （`Tab` 切换至下一列，`Shift+Tab` 切换至上一列，`Enter` 切换至下一行，`Shift+Enter` 切换至上一行。修复了 `Enter` 键在单元格内换行而非切换行的问题。）
...
- **Search functionality**（搜索功能）
  （在表格视图中新增 `Ctrl+F` 搜索功能。搜索栏浮于表格右上角，支持实时搜索匹配、大小写不敏感、上/下导航匹配项、高亮显示当前匹配单元格并自动滚动到可见区域。快捷键：`Ctrl+F` 打开搜索、`Enter` 下一个、`Shift+Enter` 上一个、`Esc` 关闭。）
